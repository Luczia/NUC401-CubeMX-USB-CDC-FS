...
static struct{
	uint8_t IsCommandDataReceived; //anynumber >0 means data were received. 0 means no data is available
	uint32_t UserRxBufferFS[APP_RX_DATA_SIZE];//it could save <MaxCommandsInBuffer> number of commands
	uint32_t UserBufferRxLength; //save the len of each command
} s_RxBuffers;
